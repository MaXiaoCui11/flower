		...
		*/
		...
		if( option.user != undefined ){
			option.user.blur(function(){
				...
				if( reg.test(str) ){
					...
					obj.user = false;
					option.tip.parent().slideDown()
					clearTimeout(publi.timer)
					publi.timer = setTimeout(function(){
						option.tip.parent().slideUp()
					},1000)
				}
			})
		}
		...
		if( option.codebox != undefined && option.code != undefined ){
			option.codebox.click(function(){
				publi.codeStr = publi.randomCode();
				option.codebox.prev().children().html(publi.codeStr);
			})
			option.code.blur(function(){
				if( $(this).val() != publi.codeStr ){
