    ...
    class Banner {
        ...
        init(ele){
            ...
            if( this.mark != undefined ){
              this.mark.on("mouseenter",function(){
        ...
        anima(){
          if( this.effect=="fade" ){
            this.mark.eq(this.index).addClass("activeMark").siblings().removeClass("activeMark");
            this.list.eq(this.index).stop().fadeIn().siblings().stop().fadeOut();
          }
          if( this.effect=="gundong" ){
            this.list.parent().stop().animate({
              marginLeft:-this.index*this.list.eq(0).width()+"px"
            })
          }
        }
        autoPlay(){
